...
entity wideband_fft_top is
	generic(
        ...
        extra_ctrl_sigs: boolean := true;                  -- = Will just instantiate a r2_wide_fft if false, else a fft_wide_unit
...
architecture RTL of wideband_fft_top is
        ...
        signal in_fft_re_arr : t_fft_slv_arr_in(wb_factor -1 downto 0);
        signal in_fft_im_arr : t_fft_slv_arr_in(wb_factor -1 downto 0);
        signal out_fft_re_arr : t_fft_slv_arr_out(wb_factor -1 downto 0);
        signal out_fft_im_arr : t_fft_slv_arr_out(wb_factor -1 downto 0);
        constant c_pft_pipeline : t_fft_pipeline := c_fft_pipeline;
        constant c_fft_pipeline : t_fft_pipeline := c_fft_pipeline;
        begin
        gen_extra_ctrl_sigs: if extra_ctrl_sigs = true generate
        ...
        gen_thin_sigs: if extra_ctrl_sigs = false generate
          r2_fft_unit : entity work.fft_wide_unit_thin
          generic map (
            g_fft          		=> cc_fft,    -- generics for the WFFT
            g_pft_pipeline 		=> c_pft_pipeline,
            g_fft_pipeline 		=> c_fft_pipeline
          )
          port map (
            ...
            in_valid   			=> in_valid,
            shiftreg  			=> in_shiftreg,
            in_fft_re_arr  	=> in_fft_re_arr,
            in_fft_im_arr   => in_fft_im_arr,
            out_valid 			=> out_valid,
            ovflw 			    => out_ovflw,
            out_fft_re_arr	=> out_fft_re_arr,
            out_fft_im_arr  => out_fft_im_arr
          );
        end generate;
        
        xtra_inprtmap: if extra_ctrl_sigs = true generate
        ...
        thin_inprtmap: if extra_ctrl_sigs = false generate
          in_fft_re_arr(0) <= RESIZE_SVEC(in_re_0, in_fft_sosi_arr(0).re'length);
          in_fft_im_arr(0) <= RESIZE_SVEC(in_im_0, in_fft_sosi_arr(0).im'length);
          out_re_0 <= RESIZE_SVEC(out_fft_re_arr(0),out_dat_w);
          out_im_0 <= RESIZE_SVEC(out_fft_im_arr(0),out_dat_w);
        end generate;
